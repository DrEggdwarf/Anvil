...
async def compile_source(
    ...
    _stdout, stderr, rc = await _subprocess.execute(cmd, timeout=60.0, cwd=workspace, env=extra_env or None)
    if rc != 0:
...
async def make_elf_from_asm(
    session_id: str, body: PwnMakeElfFromAsmRequest, sm: SessionManager = Depends(get_session_manager)
):
    bridge = _get_pwn_bridge(session_id, sm)
...
async def shellcraft_asm(
    session_id: str, body: PwnShellcraftRequest, sm: SessionManager = Depends(get_session_manager)
):
    bridge = _get_pwn_bridge(session_id, sm)
...
async def rop_find_gadget(
    session_id: str, body: PwnRopGadgetRequest, sm: SessionManager = Depends(get_session_manager)
):
    bridge = _get_pwn_bridge(session_id, sm)
...
async def encode_shellcode(
    session_id: str, body: PwnEncodeShellcodeRequest, sm: SessionManager = Depends(get_session_manager)
):
    bridge = _get_pwn_bridge(session_id, sm)
...
async def list_constants(
    session_id: str, body: PwnListConstantsRequest, sm: SessionManager = Depends(get_session_manager)
):
    bridge = _get_pwn_bridge(session_id, sm)
