...
async def scan_filtered(
    session_id: str, body: FirmwareScanFilteredRequest, sm: SessionManager = Depends(get_session_manager)
):
    bridge = _get_firmware_bridge(session_id, sm)
...
async def scan_filesystems(
    session_id: str, body: FirmwareScanRequest, sm: SessionManager = Depends(get_session_manager)
):
    bridge = _get_firmware_bridge(session_id, sm)
...
async def extract_recursive(
    session_id: str, body: FirmwareExtractRequest, sm: SessionManager = Depends(get_session_manager)
):
    bridge = _get_firmware_bridge(session_id, sm)
...
async def search_raw(
    session_id: str, body: FirmwareSearchRawRequest, sm: SessionManager = Depends(get_session_manager)
):
    bridge = _get_firmware_bridge(session_id, sm)
...
async def list_extracted(
    session_id: str, directory: str | None = None, sm: SessionManager = Depends(get_session_manager)
):
    bridge = _get_firmware_bridge(session_id, sm)
