...
async def read_discrete_inputs(session_id: str, body: ModbusReadRequest,
...
async def read_holding_registers(session_id: str, body: ModbusReadRequest,
        sm: SessionManager = Depends(get_session_manager)):
    bridge = _get_protocol_bridge(session_id, sm)
...
async def read_input_registers(session_id: str, body: ModbusReadRequest,
        sm: SessionManager = Depends(get_session_manager)):
    bridge = _get_protocol_bridge(session_id, sm)
...
async def read_exception_status(session_id: str, body: ModbusDiagRequest,
        sm: SessionManager = Depends(get_session_manager)):
    bridge = _get_protocol_bridge(session_id, sm)
...
async def read_file_record(session_id: str, body: ModbusFileRecordRequest,
        sm: SessionManager = Depends(get_session_manager)):
    bridge = _get_protocol_bridge(session_id, sm)
...
async def write_register(session_id: str, body: ModbusWriteRegisterRequest,
        sm: SessionManager = Depends(get_session_manager)):
    bridge = _get_protocol_bridge(session_id, sm)
...
async def write_coils(session_id: str, body: ModbusWriteCoilsRequest,
        sm: SessionManager = Depends(get_session_manager)):
    bridge = _get_protocol_bridge(session_id, sm)
...
async def write_registers(session_id: str, body: ModbusWriteRegistersRequest,
        sm: SessionManager = Depends(get_session_manager)):
    bridge = _get_protocol_bridge(session_id, sm)
...
async def readwrite_registers(session_id: str, body: ModbusReadWriteRequest,
        sm: SessionManager = Depends(get_session_manager)):
    bridge = _get_protocol_bridge(session_id, sm)
    return await bridge.readwrite_registers(
        body.read_address, body.read_count, body.write_address,
        body.write_values, body.device_id,
    )

@router.post("/{session_id}/write/file-record", response_model=ModbusWriteResponse)
async def write_file_record(session_id: str, body: ModbusFileRecordRequest,
        sm: SessionManager = Depends(get_session_manager)):
    bridge = _get_protocol_bridge(session_id, sm)
...
async def device_info(session_id: str, body: ModbusDeviceInfoRequest,
        sm: SessionManager = Depends(get_session_manager)):
    bridge = _get_protocol_bridge(session_id, sm)
    return await bridge.read_device_info(body.read_code, body.object_id, body.device_id)

@router.post("/{session_id}/device/report-id")
async def report_server_id(session_id: str, body: ModbusDeviceIdRequest,
        sm: SessionManager = Depends(get_session_manager)):
    bridge = _get_protocol_bridge(session_id, sm)
...
async def diag_restart(session_id: str, body: ModbusDiagRestartRequest,
        sm: SessionManager = Depends(get_session_manager)):
    bridge = _get_protocol_bridge(session_id, sm)
...
async def diag_read_register(session_id: str, body: ModbusDiagRequest,
        sm: SessionManager = Depends(get_session_manager)):
    bridge = _get_protocol_bridge(session_id, sm)
...
async def diag_force_listen_only(session_id: str, body: ModbusDiagRequest,
        sm: SessionManager = Depends(get_session_manager)):
    bridge = _get_protocol_bridge(session_id, sm)
...
async def diag_clear_counters(session_id: str, body: ModbusDiagRequest,
        sm: SessionManager = Depends(get_session_manager)):
    bridge = _get_protocol_bridge(session_id, sm)
...
async def diag_bus_message_count(session_id: str, body: ModbusDiagRequest,
        sm: SessionManager = Depends(get_session_manager)):
    bridge = _get_protocol_bridge(session_id, sm)
...
async def diag_bus_error_count(session_id: str, body: ModbusDiagRequest,
        sm: SessionManager = Depends(get_session_manager)):
    bridge = _get_protocol_bridge(session_id, sm)
...
async def diag_bus_exception_count(session_id: str, body: ModbusDiagRequest,
        sm: SessionManager = Depends(get_session_manager)):
    bridge = _get_protocol_bridge(session_id, sm)
...
async def diag_device_message_count(session_id: str, body: ModbusDiagRequest,
        sm: SessionManager = Depends(get_session_manager)):
    bridge = _get_protocol_bridge(session_id, sm)
...
async def diag_no_response_count(session_id: str, body: ModbusDiagRequest,
        sm: SessionManager = Depends(get_session_manager)):
    bridge = _get_protocol_bridge(session_id, sm)
...
async def diag_overrun_count(session_id: str, body: ModbusDiagRequest,
        sm: SessionManager = Depends(get_session_manager)):
    bridge = _get_protocol_bridge(session_id, sm)
...
async def convert_from_registers(session_id: str, body: ModbusConvertFromRequest,
        sm: SessionManager = Depends(get_session_manager)):
    bridge = _get_protocol_bridge(session_id, sm)
...
async def convert_to_registers(session_id: str, body: ModbusConvertToRequest,
        sm: SessionManager = Depends(get_session_manager)):
    bridge = _get_protocol_bridge(session_id, sm)
...
async def scan_devices(session_id: str, body: ModbusScanDevicesRequest,
        sm: SessionManager = Depends(get_session_manager)):
    bridge = _get_protocol_bridge(session_id, sm)
...
async def scan_registers(session_id: str, body: ModbusScanRegistersRequest,
        sm: SessionManager = Depends(get_session_manager)):
    bridge = _get_protocol_bridge(session_id, sm)
...
async def start_server(session_id: str, body: ModbusServerStartRequest,
        sm: SessionManager = Depends(get_session_manager)):
    bridge = _get_protocol_bridge(session_id, sm)
